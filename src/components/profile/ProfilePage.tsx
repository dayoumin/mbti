      ...
      {[1, 2, 3].map(i => (
        <div key={i} className="bg-slate-50 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[1, 2, 3, 4].map(j => (
              <div key={j} className="h-24 bg-slate-200 rounded-xl" />
            ))}
          </div>
        </div>
      ))}
...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] pb-20 lg:pb-4 lg:ml-60 xl:mr-80">
      {/* 헤더 - 모바일에서만 표시 */}
      <header className="sticky top-0 z-10 bg-slate-50/80 backdrop-blur-md border-b border-subtle lg:hidden">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="font-bold text-primary">내 프로필</h1>
          <button
            onClick={handleShare}
