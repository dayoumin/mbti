...
export class InsightService {
    constructor() {
        ...
        // 상수 참조 (방어 코드 포함)
        this.MAX_SCORE = CHEMI_CONSTANTS?.MAX_SCORE_PER_QUESTION ?? 5;
        this.DEFAULT_QUESTION_COUNT = CHEMI_CONSTANTS?.DEFAULT_QUESTION_COUNT ?? 5;
    ...
    async generateInsights() {
        let results;
        try {
            results = await resultService.getMyResults();
        } catch (error) {
    ...
    analyzeAnimalCompatibility(humanResult, animalResults) {
        ...
        for (const animal of animalResults) {
            ...
            const config = SUBJECT_CONFIG?.[animal.type];
    ...
    analyzeLifestyle(lifestyleResults) {
        ...
        for (const item of lifestyleResults) {
            ...
            const config = SUBJECT_CONFIG?.[item.type];
    ...
    getRecommendations(completedTests) {
        ...
        // CHEMI_DATA는 import된 객체 사용
        const recommendations = incomplete
            .sort((a, b) => priority[a] - priority[b])
            .slice(0, 3)
            .map(testType => {
                const config = SUBJECT_CONFIG?.[testType];
                const data = CHEMI_DATA?.[testType];
