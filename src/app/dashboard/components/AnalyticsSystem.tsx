...
const STATUS_STYLES = {
  ...
  planned: { bg: 'bg-slate-100', text: 'text-slate-600', label: '예정' },
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-semibold text-slate-900">추적 이벤트</h3>
              </div>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-indigo-600">{implementedEvents}</span>
                <span className="text-slate-500 mb-1">/ {totalEvents} 구현</span>
              </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-semibold text-slate-900">KPI 측정</h3>
              </div>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-green-600">{implementedKPIs}</span>
                <span className="text-slate-500 mb-1">/ {totalKPIs} 가능</span>
              </div>
              <div className="mt-2 w-full bg-slate-200 rounded-full h-2">
                <div
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-semibold text-slate-900">로드맵 진행</h3>
              </div>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-purple-600">{donePhases}</span>
                <span className="text-slate-500 mb-1">/ {totalPhases} Phase</span>
              </div>
              <div className="mt-2 w-full bg-slate-200 rounded-full h-2">
                <div
          ...
          {/* 권장 사항 */}
          <div>
            <h3 className="font-semibold text-slate-900 mb-3">다음 단계 권장</h3>
      ...
      {activeTab === 'events' && (
        <div className="space-y-4">
          ...
          {['test', 'next_action', 'content', 'share', 'acquisition'].map(type => {
            ...
            return (
              <div key={type} className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-slate-700">기능</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-700">GA4</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-700">자체 시스템</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-700">권장</th>
                </tr>
              </thead>
      ...
      {activeTab === 'roadmap' && (
        <div className="space-y-6">
          {ANALYTICS_ROADMAP.map(phase => (
            <div key={phase.id} className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden">
              <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                ...
                <span className="text-sm text-slate-500">{phase.description}</span>
              </div>
              <div className="divide-y divide-slate-100">
...
function EventRow({ event }: { event: TrackingEvent }) {
  return (
    <div className="px-4 py-3 flex items-start gap-3">
      <span className={`mt-1 w-2 h-2 rounded-full ${event.implemented ? 'bg-green-500' : 'bg-slate-300'
        }`} />
      <div className="flex-1">
        <div className="flex items-center gap-2">
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${kpi.implemented ? 'bg-green-500' : 'bg-slate-300'
              }`} />
...
function GAComparisonRow({ item }: { item: GAComparison }) {
  return (
    <tr>
      ...
      <td className="px-4 py-3 text-slate-600">{item.googleAnalytics}</td>
      <td className="px-4 py-3 text-slate-600">{item.ourSystem}</td>
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 flex items-start gap-3">
      <div className={`p-2 rounded-lg ${recommendation.priority === 'high' ? 'bg-red-100' :
        recommendation.priority === 'medium' ? 'bg-yellow-100' : 'bg-slate-100'
        }`}>
        <Zap className={`w-4 h-4 ${recommendation.priority === 'high' ? 'text-red-600' :
          recommendation.priority === 'medium' ? 'text-yellow-600' : 'text-slate-600'
          }`} />
      </div>
      <div className="flex-1">
        <h4 className="font-medium text-slate-900">{recommendation.title}</h4>
        <p className="text-sm text-slate-600 mt-0.5">{recommendation.description}</p>
