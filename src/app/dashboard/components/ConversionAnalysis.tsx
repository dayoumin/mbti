  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-900">전환 분석</h2>
        ...
        <button
          onClick={loadData}
          className="flex items-center gap-2 px-3 py-1.5 text-sm text-slate-600 hover:text-slate-900 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors"
        >
        ...
        <>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-6">
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600">전체 전환율 (세션 → 완료)</span>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="text-left py-2 px-3 font-medium text-slate-600">소스</th>
                      <th className="text-right py-2 px-3 font-medium text-slate-600">세션</th>
                      <th className="text-right py-2 px-3 font-medium text-slate-600">시작</th>
                      <th className="text-right py-2 px-3 font-medium text-slate-600">완료</th>
                      <th className="text-right py-2 px-3 font-medium text-slate-600">공유</th>
                  ...
                  <tbody>
                    {funnel.bySource.map((source) => (
                      <tr key={source.source} className="border-b border-slate-100 hover:bg-slate-50">
                        ...
                        <td className="text-right py-2 px-3 text-slate-900">{source.sessions}</td>
                        <td className="text-right py-2 px-3 text-slate-600">{source.testStarts}</td>
                        <td className="text-right py-2 px-3 text-slate-600">{source.testCompletes}</td>
                        <td className="text-right py-2 px-3 text-slate-600">{source.shares}</td>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {testConversion.map((test) => (
                  ...
                  >
                    <div className="font-medium text-slate-900 mb-3">{test.testType}</div>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-slate-500">시작 → 완료</span>
                      ...
                      <div className="flex justify-between mt-2">
                        <span className="text-slate-500">완료 → 공유</span>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-200 rounded-xl p-6">
              ...
              <div className="flex items-center gap-8">
                <div className="text-center">
                  ...
                  <div className="text-sm text-slate-500 mt-1">K-Factor</div>
                </div>

                <div className="flex-1 space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-600">공유한 사용자</span>
                    <span className="font-medium">{viral.sharers}명</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-600">공유로 유입된 사용자</span>
