 ...
 *
 * 학술 근거:
 * - NASA Clean Air Study (Wolverton, 1989): 공기정화 식물
 * - 원예치료 연구 (Söderback et al., 2004)
 * - RHS 식물 관리 난이도 분류
 *
 * 차원 설계:
 * - care: 관리 여유 (시간/관심)
...
console.log('🌱 반려식물 매칭 테스트 추가 스크립트\n');

// 현재 data.js 읽기
let content = fs.readFileSync(dataPath, 'utf-8');

// CHEMI_DATA에 plant 추가 확인
if (content.includes('"title": "나의 반려식물 찾기"')) {
    console.log('⚠️  plant 데이터가 이미 존재합니다. 건너뜁니다.');
} else {
    // idealType의 resultLabels 배열 끝을 찾아 그 다음에 추가
    // CHEMI_DATA의 idealType 끝 부분을 찾음
    const idealTypePattern = /(idealType:\s*\{[\s\S]*?"resultLabels":\s*\[[\s\S]*?\]\s*\})/;

    if (idealTypePattern.test(content)) {
        const plantDataStr = JSON.stringify(plantData, null, 8);
        content = content.replace(
            idealTypePattern,
            `$1,\n    plant: ${plantDataStr}`
        );
        console.log('✅ CHEMI_DATA에 plant 추가 완료');
    } else {
        console.error('❌ idealType을 찾을 수 없습니다.');
        process.exit(1);
    }
}

// SUBJECT_CONFIG에 plant 추가 확인
if (content.includes('plant:') && content.includes('"label": "식물"')) {
    console.log('⚠️  SUBJECT_CONFIG에 plant가 이미 존재합니다. 건너뜁니다.');
} else {
    // idealType config 뒤에 plant config 추가
    const configEndPattern = /(idealType\s*:\s*\{[^}]*"deepButtonText"[^}]*\})\s*(\n\s*\};)/;

    if (configEndPattern.test(content)) {
        const plantConfigStr = JSON.stringify(plantConfig, null, 8).replace(/^/gm, '    ').trim();
        content = content.replace(
            configEndPattern,
            `$1,\n    plant: ${plantConfigStr}\n$2`
        );
        console.log('✅ SUBJECT_CONFIG에 plant 추가 완료');
    } else {
        console.log('⚠️  SUBJECT_CONFIG 패턴을 찾지 못했습니다. 수동 확인 필요.');
    }
}
...
// 검증
const newContent = fs.readFileSync(dataPath, 'utf-8');
const hasPlantData = newContent.includes('"title": "나의 반려식물 찾기"');
const hasPlantConfig = newContent.includes('"label": "식물"');
const hasPlantIcon = newContent.includes('"icon": "PlantIcon"');

console.log('\n📋 검증 결과:');
console.log(`  - CHEMI_DATA.plant: ${hasPlantData ? '✅' : '❌'}`);
console.log(`  - SUBJECT_CONFIG.plant: ${hasPlantConfig ? '✅' : '❌'}`);
console.log(`  - PlantIcon 참조: ${hasPlantIcon ? '✅' : '❌'}`);

// 데이터 요약
console.log('\n📊 데이터 요약:');
console.log(`  - 차원: ${Object.keys(plantData.dimensions).length}개`);
console.log(`  - 기본 질문: ${plantData.questions.length}개`);
console.log(`  - 심화 질문: ${plantData.questions_deep.length}개`);
console.log(`  - 결과 유형: ${plantData.resultLabels.length}개`);
