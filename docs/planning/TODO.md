...
### 현재 완료
- ✅ GamificationService.ts 기본 구현
- ✅ 98개 배지 정의 (6개 카테고리: expert, community, duel, percentile, streak, special)
- ✅ 5단계 티어 시스템 (Bronze → Diamond)
- ✅ 9개 대상별 전문가 트랙 (cat, dog, rabbit, hamster, fish, bird, reptile, coffee, plant)
- ✅ 코드 리뷰 및 버그 5개 수정

### Phase 2: 퀴즈 대결 기능
- **우선순위**: 높음
- **상태**: 미착수
- **설명**: duel 카테고리 배지가 동작하려면 퀴즈 대결 기능 필요

**필요 작업:**
1. 대결 매칭 시스템 (실시간 or 비동기)
2. 대결 UI 컴포넌트
3. `recordDuelResult()` 호출 연동
4. 승률, 연승, 역전승 등 통계 집계

### Phase 3: 비율 배지 서버 계산
- **우선순위**: 중간
- **상태**: 미착수
- **설명**: Top N% 배지는 서버에서 전체 사용자 대비 계산 필요

**필요 작업:**
1. Supabase에 사용자 통계 집계 테이블
2. 주기적 비율 계산 (weekly/monthly)
3. 클라이언트에서 비율 배지 조회

### Phase 4: 프로필 UI 개선
- **우선순위**: 중간
- **상태**: 미착수
- **설명**: 98개 배지를 효과적으로 표시하는 UI 필요
