...
#### 5.4 게이미피케이션
- [x] **배지 시스템** - 테스트 완료, 케미 분석 횟수별 배지
- [x] **레벨 시스템** - 활동에 따른 레벨업
- [ ] **성격 컬렉션** - 만난 성격 유형 도감
- [ ] **업적 시스템** - "모든 성격 유형과 케미 분석 완료!"

#### 5.5 퀴즈/게임 시스템 ★ NEW
- [x] **지식 퀴즈** - 반려동물 관련 상식 퀴즈
- [x] **VS 투표** - 양자택일 투표 (예: 츄르 vs 간식)
- [x] **스트릭 시스템** - 연속 참여 일수 추적
- [x] **핫 토픽** - 인기 콘텐츠 하이라이트
- [x] **카테고리 진행률** - 완료율 시각화
- [ ] **일일 퀴즈** - 매일 새로운 퀴즈 3문제
- [ ] **퀴즈 토너먼트 대회**
  ...
  - 리더보드 및 시상 (배지, 타이틀)
  - 친구 초대 대결 기능
- [ ] **퀴즈 난이도 시스템** - 쉬움/보통/어려움
- [ ] **퀴즈 제작 기능** - 사용자가 만드는 퀴즈
- [ ] **카테고리 마스터** - 카테고리 전문가 칭호
...
#### 5.5.5 콘텐츠 DB화 (예정, 콘텐츠 100개+ 시)
- [ ] **퀴즈/투표/토너먼트 Supabase 이관** - 현재 코드 하드코딩 → DB 저장
- [ ] **콘텐츠 관리 API** - CRUD 엔드포인트
- [ ] **관리자 CMS** - 대시보드에서 콘텐츠 추가/수정
- [ ] **동적 콘텐츠 로딩** - 배포 없이 콘텐츠 추가 가능

#### 5.5.6 콘텐츠 요청 생태계 (예정) ★ NEW
> 사용자가 원하는 콘텐츠를 요청하고, 투표로 우선순위를 정하고, 플랫폼이 제작

**플라이휠**: 요청 → 투표 → 선정(리스트 제거) → 제작 → 출시 → 참여 → 새 요청

- [ ] **요청 시스템**
  - [ ] 요청 제출 UI (카테고리 선택 + 제목 + 설명)
  - [ ] 제한: 사용자당 주간 3개, 제목 5~50자, 설명 10~200자
  - [ ] 중복 감지: 제목 유사도 70% 이상 시 병합 제안
  - [ ] 필터링: 욕설, 비방, 홍보, 저작권 침해, 실현 불가능
  - [ ] 수정: 24시간 내 수정 가능 (투표 시작 전)
  - [ ] 취소: 투표 10표 미만일 때만 취소 가능
- [ ] **투표 시스템**
  - [ ] 요청당 1인 1표
  - [ ] 실시간 투표 랭킹 표시
  - [ ] 투표 기간: 2주 (미달 시 보관함 이동)
- [ ] **선정 & 제작**
  - [ ] 주간 TOP 3 자동 선정 (최소 20표 이상)
  - [ ] 카테고리당 최대 2개 (다양성 확보)
  - [ ] 선정된 요청 리스트에서 제거
  - [ ] 제작 기간: 투표 1~2일, 퀴즈 3~5일, 토너먼트 5~7일, 테스트 1~2주
  - [ ] 상태 표시: 선정됨 → 제작 중 → 검수 중 → 출시 예정 → 출시 완료
- [ ] **알림 체계**
  - [ ] 요청자: 채택 시, 제작 시작 시, 출시 시
  - [ ] 투표자: 투표한 요청 채택 시, 출시 시
- [ ] **보상 체계**
  - [ ] 요청자: 채택 시 +100P + "💡 아이디어 뱅크" 배지
  - [ ] 투표자: 채택 적중 시 +20P + "👁️ 트렌드세터" 배지
  - [ ] 참여자: 24시간 내 참여 시 "⚡ 얼리버드" 배지
- [ ] **성과 기반 배지** (후순위)
  - [ ] 내 요청 콘텐츠 참여자 100명+ → "🌟 히트메이커"
  - [ ] 내 요청 콘텐츠 참여자 500명+ → "💎 플래티넘 기획자"
  - [ ] 내 요청 콘텐츠 공유 50회+ → "🔥 바이럴 마스터"
- [ ] **DB 테이블**
  - [ ] content_requests (요청 저장)
  - [ ] request_votes (투표 저장)
  - [ ] request_badges (배지 지급 기록)

**왜 이 모델인가**:
- 사용자가 직접 테스트 제작 → 품질 보장 어려움
- 요청/투표만 받고 플랫폼이 제작 → 수요 검증 + 품질 보장
- "내가 요청한 거 나왔다!" → 공유 동기 + 소속감
...
> **현재 상태**: 토너먼트는 이모지+텍스트 기반. 이미지 지원 시 확장 예정.

**이미지 제작 방식** (택 1):
- AI 이미지 생성 (DALL-E, Midjourney 등)
- Bannerbear Pro (템플릿 기반 자동 생성)

**이미지 월드컵 후보**:
| 월드컵 | 참가자 | 이미지 필요 |
|--------|--------|------------|
| 치킨 브랜드 월드컵 | 교촌, BBQ, BHC, 굽네 등 16강 | 로고/메뉴 이미지 |
| 고양이 품종 월드컵 | 러시안블루, 먼치킨 등 16강 | 품종별 사진 |
| 강아지 품종 월드컵 | 골든리트리버, 포메라니안 등 16강 | 품종별 사진 |
| 카페 브랜드 월드컵 | 스타벅스, 투썸 등 8강 | 로고 이미지 |
| 아이스크림 월드컵 | 메로나, 스크류바 등 16강 | 제품 이미지 |

**구현 작업**:
- [ ] Tournament 타입에 `imageUrl` 필드 추가
- [ ] TournamentCard 컴포넌트 이미지 렌더링
- [ ] 이미지 저장소 설정 (Supabase Storage 또는 CDN)
- [ ] 이미지 생성 파이프라인 구축
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2024-12-22 | Phase 5.5 퀴즈/게임 시스템 추가 (토너먼트 대회 포함) |
...
| 2025-12-23 | Phase 5.5.6 콘텐츠 요청 생태계 기획 추가 |
