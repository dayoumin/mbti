/**
 * InsightView - 통합 인사이트 UI 컴포넌트
 *
 * 여러 테스트 결과를 종합하여 보여주는 화면
 */
...
(function(window) {
    ...
    const RecommendationButton = ({ rec, onSelect }) => {
        const IconComponent = rec.icon ? window[rec.icon] : null;

        return h('button', {
            onClick: () => onSelect(rec.testType),
            className: 'w-full flex items-center gap-3 p-3 bg-white rounded-xl border-2 border-gray-200 hover:border-yellow-400 hover:bg-yellow-50 transition-all text-left overflow-hidden'
        },
            IconComponent && h('div', { className: 'w-10 h-10 flex-shrink-0 overflow-hidden' },
                h(IconComponent, { mood: 'happy', className: 'w-10 h-10' })
            ),
            h('div', { className: 'flex-1 min-w-0' },
                h('p', { className: 'font-bold text-gray-800 text-sm' }, rec.title),
                h('p', { className: 'text-xs text-gray-500 truncate' }, rec.reason)
            ),
            h('span', { className: 'text-gray-400 flex-shrink-0' }, '→')
        );
    };
    ...
    // 메인 InsightView 컴포넌트
    const InsightView = ({ onClose, onSelectTest }) => {
        ...
        const loadInsights = async () => {
            setLoading(true);
            try {
                const data = await window.insightService.generateInsights();
                setInsights(data);
            } catch (error) {
        ...
        const TabButton = ({ id, label, isActive }) => h('button', {
            ...
            className: `flex-1 py-2 px-3 text-sm font-bold transition-colors ${
                isActive
        ...
        return h('div', {
            ...
            h('div', { className: 'max-w-md mx-auto p-4 pb-20' },
                ...
                h('div', { className: 'flex justify-between items-center mb-4' },
                    ...
                    h('h2', { className: 'text-xl font-bold text-gray-800' }, '🔮 통합 인사이트'),
                    h('div', { className: 'w-12' }) // 균형을 위한 빈 공간
                ),
