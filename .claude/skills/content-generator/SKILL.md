...
- [ ] **시간 민감도 설정 (금액/트렌드 콘텐츠)**
- [ ] 빌드 에러 없음
- [ ] **index.ts에 등록됨**

## 시간 민감도 (Time Sensitivity)

금액, 수치, 트렌드 기반 콘텐츠는 유효기간 설정 필수!

### 민감도 레벨

| 레벨 | 유효기간 | 예시 |
|------|---------|------|
| `high` | 2년 | 축의금 평균, 물가, 연봉 |
| `medium` | 3년 | MZ 인식, 선호도, 유행어 |
| `low` | 4년 | 밸런스게임, 상황별 반응 |
| `none` | 무기한 | 동물 지식, MBTI, 성격 |
...
```typescript
{
  ...
  meta: {
    timeSensitivity: {
      sensitivity: 'high',     // 레벨
      sourceYear: 2025,        // 데이터 기준 연도
      // validUntil: '2027-12' // 자동 계산됨
    }
  }
}
```
...
- **high**: 금액, 물가, 연봉, 통계 수치
- **medium**: 세대 인식, 유행어, 트렌드
- **low**: 상황 판단, 밸런스게임, 에티켓
- **none**: 과학적 사실, 동물 특성, 성격
