...
description: 퀴즈/투표/상황반응 콘텐츠 생성. "고양이 퀴즈 10개 만들어줘", "상황반응 5개 만들어줘"
keywords:
  - 퀴즈 생성
  - 퀴즈 만들어
  - 투표 생성
...
**이 에이전트는 참여형/스낵 콘텐츠(3종)를 생성합니다:**

| 타입 | 설명 | 이 에이전트 담당 |
|------|------|-----------------|
| Test | 성향 테스트 (다차원) | ❌ → CLAUDE.md `신규 테스트 추가` 참조 |
| Matching | 궁합 테스트 | ❌ → CLAUDE.md `신규 테스트 추가` 참조 |
| **Quiz** | 지식 퀴즈 (정답 있음) | ✅ |
| **Poll** | 투표 (VS, 선택) | ✅ |
| **Reaction** | 상황별 반응 | ✅ |
...
5. **시간 민감도**: 금액/수치/트렌드 기반 콘텐츠는 `meta.timeSensitivity` 필수
...
7. **시간 민감도**: 금액/트렌드 콘텐츠는 유효기간 설정 필수 (아래 참고)

## 시간 민감도 설정 (Time Sensitivity)

금액, 수치, 트렌드 기반 콘텐츠는 시간이 지나면 정보가 outdated됩니다.
콘텐츠 생성 시 민감도를 판단하여 `meta.timeSensitivity` 설정 필수!

### 민감도 레벨 및 유효기간

| 레벨 | 유효기간 | 콘텐츠 유형 | 예시 |
|------|---------|------------|------|
| `high` | 2년 | 금액/수치 기반 | 축의금 평균, 물가, 연봉 |
| `medium` | 3년 | 트렌드 기반 | MZ 인식, 선호도, 유행어 |
| `low` | 4년 | 상황 판단 | 밸런스게임, 상황별 반응 |
| `none` | 무기한 | 보편적 성향 | MBTI, 성격, 동물 지식 |
...
{
  id: 'money-quiz-001',
  question: '2025년 MZ세대 평균 축의금은?',
  meta: {
    timeSensitivity: {
      sensitivity: 'high',
      sourceYear: 2025,
      // validUntil 자동 계산: '2027-12'
    }
  }
}
...
// 상황 판단 (4년 유효)
{
  ...
  situation: '친구가 비싼 메뉴만 시키고 N빵 하자고 한다',
  meta: {
    timeSensitivity: {
      sensitivity: 'low',
      sourceYear: 2025,
...
{
  id: 'cat-quiz-001',
  question: '고양이가 꼬리를 세우면?',
  // timeSensitivity 불필요 (기본: none)
}
```
...
1. **sourceYear 필수**: 데이터 기준 연도 반드시 명시
2. **애매하면 높은 레벨**: high와 medium 중 고민되면 high 선택
3. **유효기간 임박 경고**: 6개월 전부터 대시보드에 검토 필요 표시
4. **만료된 콘텐츠**: 자동으로 노출 중지됨
